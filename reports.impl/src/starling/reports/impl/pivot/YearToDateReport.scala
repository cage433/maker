 ...
 */
class YearToDateReport(reportContextBuilder: ReportContextBuilder, curveIdentifierFactory: CurveIdentifierFactory) {

  private val valuescache = CacheFactory.getCache("YearToDateReportAspectValuesCache")
  private val cache = CacheFactory.getCache("YearToDateReportCache")
  ...
  private def aspectValues(desk: Desk) = valuescache.memoize((desk), {
    ...
    tradeSets.map {
      case (tradeSet, ts) => {
        new UnfilteredPivotTableDataSource() {
          val fields = yearToDateField :: aspectCOY :: aspectCOYC :: Nil

          def unfilteredData(pfs: PivotFieldsState) = {
            rows(tradeSet, ts)
          }
          def fieldDetailsGroups = {
            List(FieldDetailsGroup("Report Fields", fields))
          }
        }
      }
    }
