...
import starling.pivot.{Field, AmendPivotEdit, DeletePivotEdit, PivotEdit}
import starling.utils.ImplicitConversions._
import starling.db.{MarketDataReader, MarketDataStore}
import starling.marketdata._

class WithEditsMarketDataReader(reader:MarketDataReader, marketDataType:MarketDataType, allEdits:Set[PivotEdit]) extends MarketDataReader {
  ...
  val newMarketData:List[(TimedMarketDataKey, MarketData)] = groupedEdits.flatMap{ case ((point,key),edits) => {
    reader.read(key.dataType, Some(Set(point.day)), Some(Set(point.timeOfDay)), Some(Set(key))).headOption match {
      case Some(_) => Nil
      case None => List( (TimedMarketDataKey(point,key), marketDataType.createValue(edits.toList.map(_.values))) )
  } } }.toList
  ...
  val allDeletes = allEdits.toList.filterCast[DeletePivotEdit].map { case delete => {
    ...
    val key: MarketDataKey = marketDataType.createKey(delete.values)
        ...
        groupedEdits.get((point, key)) match {
          case Some(edits) => {
            ...
            val currentRowsWithDeletesRemoved = currentRows.filterNot(currentMap => {
              deletes.exists(del => del.matches(currentMap, observationDayField, observationTimeField))
            })
