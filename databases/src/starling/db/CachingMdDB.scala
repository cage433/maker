...
class CachingMdDB(adapted: MdDB) extends AdaptingMdDB(adapted) {
  private val cache = CacheFactory.getCache("mddb-cache", unique=true)
  ...
  override def query(version: Int, mds: List[MarketDataSet], marketDataType: MarketDataTypeName,
    ...
    val result: List[(TimedMarketDataKey, MarketData)] = cache.memoize((version, mds.toSet, marketDataType), {
      super.query(version, mds, marketDataType, None, None, None)
    })
