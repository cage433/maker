...
class TitanTradeAttributesTest extends StarlingSpec with ShouldMatchers {
  ...
  val tradeAttributes = TitanTradeAttributes("quota1", Quantity(1, UOM.MT), "trade id", "gc", "comment", Day(2011, 1, 1), "shape", "finall", Percentage.EPSILON, Percentage.EPSILON, List("id1"))
  ...
  "should be equal with different event ids" in {
    val b = tradeAttributes.copy(eventIDs = List("id2"))
  ...
  "Titan Trade should be equal with equal trade fields and attributes" in {
    ...
    val tradeB = titanTestTrade.copy(
      attributes = tradeAttributes.copy(eventIDs = List("id2"))
    )
