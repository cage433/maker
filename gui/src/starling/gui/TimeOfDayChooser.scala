...
class TimeOfDayChooser(timeOfDay0: TimeOfDay = TimeOfDay.StartOfDay)
  ...
  protected def publish() : Unit = publish(TimeOfDayChangedEvent(this, timeOfDay))

  def timeOfDay : TimeOfDay = value
  def timeOfDay_=(timeOfDay:TimeOfDay) = value = timeOfDay
}

case class TimeOfDayChangedEvent(source: Component, timeOfDay: TimeOfDay) extends Event
