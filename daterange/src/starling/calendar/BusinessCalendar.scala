...
trait BusinessCalendar { self =>
  ...
  def thisOrPreviousBusinessDay(day: Day): Day = if(isBusinessDay(day)) day else previousBusinessDay(day)
  def thisOrNextBusinessDay(day: Day): Day     = if(isBusinessDay(day)) day else nextBusinessDay(day)
