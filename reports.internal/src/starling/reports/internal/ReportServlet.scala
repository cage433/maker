    ...
    if (path == "" || path == "/") {
      ...
      userReportsService.allUserReports.foreach { case(user,reports) => {
        reports.foreach { report=> {
          writer.write( <a href={"/reports/" + user.urlEncode + "/" + report.reportName.urlEncode + "/" + ReportServlet.defaultDay}>{user} {report.reportName} {ReportServlet.defaultDay.toString}</a>.toString )
          writer.write( " <br>\n" )
        } }
      } }
    } else if (path == "/list") {
