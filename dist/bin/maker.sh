...
echo "Bootstrap " + $MAKER_OWN_BOOTSTRAP_DIR
...
main() {
  ...
  then
    ...
    CLASSPATH="$(maker_internal_classpath):$(external_jars):${scala_jars}:$MAKER_OWN_ROOT_DIR/resources/"
...
#    echo "Args = $MAKER_ARGS"
    ...
    then
      ...
      if [ -e $MAKER_COMPILED_PROJ_OUTPUT_DIR ] && [ ! -e $MAKER_COMPILED_PROJ_OUTPUT_DIR-TMP ] && [ -z $RECOMPILE_PROJECT ]; then
        ...
        debug "java -classpath $(external_jars):$CLASSPATH -Dscala.usejavacp=true scala.tools.nsc.Main -d $MAKER_COMPILED_PROJ_OUTPUT_DIR-TMP $MAKER_COMPILED_PROJ_INPUT_FILES"
        java -classpath "$(external_jars):$CLASSPATH" -Dscala.usejavacp=true  scala.tools.nsc.Main -d $MAKER_COMPILED_PROJ_OUTPUT_DIR-TMP $MAKER_COMPILED_PROJ_INPUT_FILES | tee $MAKER_OWN_ROOT_DIR/proj-compile-output ; test ${PIPESTATUS[0]} -eq 0 || exit -1
...
scala_jars(){
  jars=`ls $MAKER_OWN_SCALA_LIB_DIR/*.jar | xargs | sed 's/ /:/g'`
  echo $jars
}
...
scala_ivy_update() {
  debug "Updating scala libraries (using Ivy)"
...
fetch_ivy() {
  ...
  if [ ! -f ${MAKER_OWN_BOOTSTRAP_DIR}/ivy-2.3.0-rc2.jar ]; then
