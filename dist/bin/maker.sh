...
update_external_jars_if_required(){
  ...
  then
    ...
    run_command "$(ivy_command $MAKER_OWN_ROOT_DIR/.maker/ivy/scala-compiler-ivy.xml $MAKER_OWN_LIB_DIR) -types jar sync" || exit -1
    run_command "$(ivy_command $MAKER_OWN_ROOT_DIR/.maker/ivy/scala-library-ivy.xml $MAKER_OWN_LIB_DIR) -types jar" || exit -1
    run_command "$(ivy_command $MAKER_OWN_ROOT_DIR/utils/ivy.xml $MAKER_OWN_LIB_DIR) -types jar" || exit -1

    run_command "$(ivy_command $MAKER_OWN_ROOT_DIR/.maker/ivy/scala-library-ivy.xml $MAKER_PROJECT_SCALA_LIB_DIR) -types jar sync" || exit -1
    run_command "$(ivy_command $MAKER_OWN_ROOT_DIR/.maker/ivy/scala-library-ivy.xml $MAKER_PROJECT_SCALA_LIB_DIR) -types source" || exit -1
    run_command "$(ivy_command $MAKER_OWN_ROOT_DIR/.maker/ivy/scala-compiler-ivy.xml $MAKER_PROJECT_ZINC_LIB_DIR) -types jar sync" || exit -1
