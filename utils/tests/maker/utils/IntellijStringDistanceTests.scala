...
import maker.utils.IntellijStringDistance._

class IntellijStringDistanceTests extends FunSuite with TableDrivenPropertyChecks{
  ...
  test("intellij distance"){
    ...
    forAll(table){
      (matcher, name, shouldMatch) => 
        assert(distance(matcher, name) === shouldMatch)
    }
  }
  ...
  test("best intellij matches"){
    ...
    forAll(table){
      (matcher, names, best) => 
        assert(bestMatches(matcher, names) === best)
    }
