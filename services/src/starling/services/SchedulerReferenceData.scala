...
class SchedulerReferenceData(scheduler: Scheduler) extends UnfilteredPivotTableDataSource with ScheduledTaskAttributes {
  val group@List(name, calendar, timing, londonTime, period, sender, recipients, enabled) = fieldDetails(
    "Task", "Calendar", "Time", "London Time", "Period", "Email Sender", "Email Recipients", "Enabled")
