...
class MarketDataServiceRPC(marketDataStore: MarketDataStore) extends MarketDataService {
  def marketData(parameters: MarketDataRequestParameters): MarketDataResponse = {
    ...
    val version = parameters.version.mapOrElse(_.toInt, marketDataStore.latest(selection))

    val pivot = marketDataStore.pivot(MarketDataIdentifier(selection, version), MarketDataTypes.fromName("PriceFixings"))
    val pfs = PivotFieldsState(List(Field(parameters.measures)), fields(parameters.rows), fields(parameters.columns))

    val data = PivotTableModel.createPivotData(pivot, PivotFieldParams(true, Some(pfs))).pivotTable.toFlatRows(Totals.Null)

    MarketDataResponse(parameters.update(_.version = Some(version)), Nil, 0,
      data.map(row => MarketDataColumnData(row.map(_.toString))))
  }
