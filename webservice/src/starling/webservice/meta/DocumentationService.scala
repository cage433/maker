...
class DocumentationService extends DocumentationServiceApi {
  //RuntimeDelegate.setInstance(new ResteasyProviderFactory());
  private val serviceClasses = new ListBuffer[Class[_]] += getClass
  private val files: Map[String, File] = FileUtils.listFiles(new File("services/cs/TrinityService/files"),
  ...
  ).withDefaultValue(MediaType.valueOf("text/plain"))
  ...
  def serviceFor(serviceType: String) = WebService.fromClass(serviceClasses.find(_.getName == serviceType).getOrThrow(
    "Unknown service: %s, valid values: %s" % (serviceType, serviceClasses.map(_.getName).mkString(", "))))
