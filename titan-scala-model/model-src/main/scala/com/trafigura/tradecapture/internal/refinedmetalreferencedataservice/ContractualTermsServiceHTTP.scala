...
// JAX-RS support for ContractualTermsService


// A resource trait corresponding to ContractualTermsService

@Path("/ContractualTermsService")
trait ContractualTermsServiceResource {
  
  @Path("GetContractualTerms")
  @POST  @Produces(Array("application/json"))
  def getContractualTerms(@HeaderParam("User") _userHeader:String): Response
  
}


// Wrap a ContractualTermsServiceResource (provided by RESTEasy's client
// support) as a ContractualTermsService
//
// Note: will throw an exception on parse/status code errors, exception will contain details of the error
//
class ContractualTermsServiceResourceProxy(val resource: ContractualTermsServiceResource) extends ContractualTermsService {

  val log = org.slf4j.LoggerFactory.getLogger(this.getClass.getName)

  
  def getContractualTerms() = getContractualTerms(null)
  def getContractualTerms( securityContext:SecurityContext = null): List[com.trafigura.tradecapture.internal.refinedmetal.ContractualTerms] = {
    val _userHeader = if (securityContext != null) { securityContext.user } else { null }
    val response = resource.getContractualTerms(_userHeader)
    val content = response.asInstanceOf[ClientResponse[String]].getEntity(classOf[String])

    log.debug("Completed call to getContractualTerms: " + response.getStatus() + " - " + content)
    ...
    if (response.getStatus() == Response.Status.OK.getStatusCode()) {
          ...
          JSONConversions.optionalList(obj, x => (x) match { case null => null; case o: org.codehaus.jettison.json.JSONObject => cache.objectFor(o).getOrElse(cache.seen(o, com.trafigura.tradecapture.internal.refinedmetal.ContractualTerms.fromJson(o, cache))) })
	      } catch {
          case ex : Exception => {
...
// Wrap a ContractualTermsService as a ContractualTermsServiceResource (e.g. to be
// exposed by a JAX-RS implementation).

class ContractualTermsServiceResourceStub(val target: ContractualTermsService, filters: java.util.List[ServiceFilter])
    extends AbstractResourceStub(filters)
    with ContractualTermsServiceResource {

  def this(target: ContractualTermsService) = this(target, new java.util.ArrayList[ServiceFilter])
  ...
  def getContractualTerms(_userHeader:String): Response = {

    try {
    val params = new JSONObject()
     val res = 
      ContractualTermsServiceJSON.dispatch(target, filters.toSeq, "GetContractualTerms", params, buildSecurityContext(_userHeader))
