...
// JAX-RS support for DocumentTypeService


// A resource trait corresponding to DocumentTypeService

@Path("/DocumentTypeService")
trait DocumentTypeServiceResource {
  
  @Path("GetDocumentTypes")
  @POST  @Produces(Array("application/json"))
  def getDocumentTypes(@HeaderParam("User") _userHeader:String): Response
  
}


// Wrap a DocumentTypeServiceResource (provided by RESTEasy's client
// support) as a DocumentTypeService
//
// Note: will throw an exception on parse/status code errors, exception will contain details of the error
//
class DocumentTypeServiceResourceProxy(val resource: DocumentTypeServiceResource) extends DocumentTypeService {

  val log = org.slf4j.LoggerFactory.getLogger(this.getClass.getName)

  
  def getDocumentTypes() = getDocumentTypes(null)
  def getDocumentTypes( securityContext:SecurityContext = null): List[com.trafigura.tradecapture.internal.refinedmetal.DocumentType] = {
    val _userHeader = if (securityContext != null) { securityContext.user } else { null }
    val response = resource.getDocumentTypes(_userHeader)
    val content = response.asInstanceOf[ClientResponse[String]].getEntity(classOf[String])

    log.debug("Completed call to getDocumentTypes: " + response.getStatus() + " - " + content)
    ...
    if (response.getStatus() == Response.Status.OK.getStatusCode()) {
          ...
          JSONConversions.optionalList(obj, x => (x) match { case null => null; case o: org.codehaus.jettison.json.JSONObject => cache.objectFor(o).getOrElse(cache.seen(o, com.trafigura.tradecapture.internal.refinedmetal.DocumentType.fromJson(o, cache))) })
	      } catch {
          case ex : Exception => {
...
// Wrap a DocumentTypeService as a DocumentTypeServiceResource (e.g. to be
// exposed by a JAX-RS implementation).

class DocumentTypeServiceResourceStub(val target: DocumentTypeService, filters: java.util.List[ServiceFilter])
    extends AbstractResourceStub(filters)
    with DocumentTypeServiceResource {

  def this(target: DocumentTypeService) = this(target, new java.util.ArrayList[ServiceFilter])
  ...
  def getDocumentTypes(_userHeader:String): Response = {

    try {
    val params = new JSONObject()
     val res = 
      DocumentTypeServiceJSON.dispatch(target, filters.toSeq, "GetDocumentTypes", params, buildSecurityContext(_userHeader))
