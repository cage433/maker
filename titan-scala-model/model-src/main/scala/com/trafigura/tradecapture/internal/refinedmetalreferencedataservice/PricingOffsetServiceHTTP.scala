...
// JAX-RS support for PricingOffsetService


// A resource trait corresponding to PricingOffsetService

@Path("/PricingOffsetService")
trait PricingOffsetServiceResource {
  
  @Path("GetPricingOffsets")
  @POST  @Produces(Array("application/json"))
  def getPricingOffsets(@HeaderParam("User") _userHeader:String): Response
  
}


// Wrap a PricingOffsetServiceResource (provided by RESTEasy's client
// support) as a PricingOffsetService
//
// Note: will throw an exception on parse/status code errors, exception will contain details of the error
//
class PricingOffsetServiceResourceProxy(val resource: PricingOffsetServiceResource) extends PricingOffsetService {

  val log = org.slf4j.LoggerFactory.getLogger(this.getClass.getName)

  
  def getPricingOffsets() = getPricingOffsets(null)
  def getPricingOffsets( securityContext:SecurityContext = null): List[com.trafigura.tradecapture.internal.refinedmetal.PricingOffset] = {
    val _userHeader = if (securityContext != null) { securityContext.user } else { null }
    val response = resource.getPricingOffsets(_userHeader)
    val content = response.asInstanceOf[ClientResponse[String]].getEntity(classOf[String])

    log.debug("Completed call to getPricingOffsets: " + response.getStatus() + " - " + content)
    ...
    if (response.getStatus() == Response.Status.OK.getStatusCode()) {
          ...
          JSONConversions.optionalList(obj, x => (x) match { case null => null; case o: org.codehaus.jettison.json.JSONObject => cache.objectFor(o).getOrElse(cache.seen(o, com.trafigura.tradecapture.internal.refinedmetal.PricingOffset.fromJson(o, cache))) })
	      } catch {
          case ex : Exception => {
...
// Wrap a PricingOffsetService as a PricingOffsetServiceResource (e.g. to be
// exposed by a JAX-RS implementation).

class PricingOffsetServiceResourceStub(val target: PricingOffsetService, filters: java.util.List[ServiceFilter])
    extends AbstractResourceStub(filters)
    with PricingOffsetServiceResource {

  def this(target: PricingOffsetService) = this(target, new java.util.ArrayList[ServiceFilter])
  ...
  def getPricingOffsets(_userHeader:String): Response = {

    try {
    val params = new JSONObject()
     val res = 
      PricingOffsetServiceJSON.dispatch(target, filters.toSeq, "GetPricingOffsets", params, buildSecurityContext(_userHeader))
