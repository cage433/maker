...
// JAX-RS support for MarketService


// A resource trait corresponding to MarketService

@Path("/MarketService")
trait MarketServiceResource {
  
  @Path("GetMarkets")
  @POST  @Produces(Array("application/json"))
  def getMarkets(@HeaderParam("User") _userHeader:String): Response
  
}


// Wrap a MarketServiceResource (provided by RESTEasy's client
// support) as a MarketService
//
// Note: will throw an exception on parse/status code errors, exception will contain details of the error
//
class MarketServiceResourceProxy(val resource: MarketServiceResource) extends MarketService {

  val log = org.slf4j.LoggerFactory.getLogger(this.getClass.getName)

  
  def getMarkets() = getMarkets(null)
  def getMarkets( securityContext:SecurityContext = null): List[com.trafigura.tradecapture.internal.refinedmetal.Market] = {
    val _userHeader = if (securityContext != null) { securityContext.user } else { null }
    val response = resource.getMarkets(_userHeader)
    val content = response.asInstanceOf[ClientResponse[String]].getEntity(classOf[String])

    log.debug("Completed call to getMarkets: " + response.getStatus() + " - " + content)
    ...
    if (response.getStatus() == Response.Status.OK.getStatusCode()) {
          ...
          JSONConversions.optionalList(obj, x => (x) match { case null => null; case o: org.codehaus.jettison.json.JSONObject => cache.objectFor(o).getOrElse(cache.seen(o, com.trafigura.tradecapture.internal.refinedmetal.Market.fromJson(o, cache))) })
	      } catch {
          case ex : Exception => {
...
// Wrap a MarketService as a MarketServiceResource (e.g. to be
// exposed by a JAX-RS implementation).

class MarketServiceResourceStub(val target: MarketService, filters: java.util.List[ServiceFilter])
    extends AbstractResourceStub(filters)
    with MarketServiceResource {

  def this(target: MarketService) = this(target, new java.util.ArrayList[ServiceFilter])
  ...
  def getMarkets(_userHeader:String): Response = {

    try {
    val params = new JSONObject()
     val res = 
      MarketServiceJSON.dispatch(target, filters.toSeq, "GetMarkets", params, buildSecurityContext(_userHeader))
