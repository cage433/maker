...
// JAX-RS support for PaymentTermsService


// A resource trait corresponding to PaymentTermsService

@Path("/PaymentTermsService")
trait PaymentTermsServiceResource {
  
  @Path("GetPaymentTerms")
  @POST  @Produces(Array("application/json"))
  def getPaymentTerms(@HeaderParam("User") _userHeader:String): Response
  
}


// Wrap a PaymentTermsServiceResource (provided by RESTEasy's client
// support) as a PaymentTermsService
//
// Note: will throw an exception on parse/status code errors, exception will contain details of the error
//
class PaymentTermsServiceResourceProxy(val resource: PaymentTermsServiceResource) extends PaymentTermsService {

  val log = org.slf4j.LoggerFactory.getLogger(this.getClass.getName)

  
  def getPaymentTerms() = getPaymentTerms(null)
  def getPaymentTerms( securityContext:SecurityContext = null): List[com.trafigura.tradecapture.internal.refinedmetal.PaymentTerms] = {
    val _userHeader = if (securityContext != null) { securityContext.user } else { null }
    val response = resource.getPaymentTerms(_userHeader)
    val content = response.asInstanceOf[ClientResponse[String]].getEntity(classOf[String])

    log.debug("Completed call to getPaymentTerms: " + response.getStatus() + " - " + content)
    ...
    if (response.getStatus() == Response.Status.OK.getStatusCode()) {
          ...
          JSONConversions.optionalList(obj, x => (x) match { case null => null; case o: org.codehaus.jettison.json.JSONObject => cache.objectFor(o).getOrElse(cache.seen(o, com.trafigura.tradecapture.internal.refinedmetal.PaymentTerms.fromJson(o, cache))) })
	      } catch {
          case ex : Exception => {
...
// Wrap a PaymentTermsService as a PaymentTermsServiceResource (e.g. to be
// exposed by a JAX-RS implementation).

class PaymentTermsServiceResourceStub(val target: PaymentTermsService, filters: java.util.List[ServiceFilter])
    extends AbstractResourceStub(filters)
    with PaymentTermsServiceResource {

  def this(target: PaymentTermsService) = this(target, new java.util.ArrayList[ServiceFilter])
  ...
  def getPaymentTerms(_userHeader:String): Response = {

    try {
    val params = new JSONObject()
     val res = 
      PaymentTermsServiceJSON.dispatch(target, filters.toSeq, "GetPaymentTerms", params, buildSecurityContext(_userHeader))
