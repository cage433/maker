...
// JAX-RS support for QPTypeService


// A resource trait corresponding to QPTypeService

@Path("/QPTypeService")
trait QPTypeServiceResource {
  
  @Path("GetQPTypes")
  @POST  @Produces(Array("application/json"))
  def getQPTypes(@HeaderParam("User") _userHeader:String): Response
  
}


// Wrap a QPTypeServiceResource (provided by RESTEasy's client
// support) as a QPTypeService
//
// Note: will throw an exception on parse/status code errors, exception will contain details of the error
//
class QPTypeServiceResourceProxy(val resource: QPTypeServiceResource) extends QPTypeService {

  val log = org.slf4j.LoggerFactory.getLogger(this.getClass.getName)

  
  def getQPTypes() = getQPTypes(null)
  def getQPTypes( securityContext:SecurityContext = null): List[com.trafigura.tradecapture.internal.refinedmetal.QPType] = {
    val _userHeader = if (securityContext != null) { securityContext.user } else { null }
    val response = resource.getQPTypes(_userHeader)
    val content = response.asInstanceOf[ClientResponse[String]].getEntity(classOf[String])

    log.debug("Completed call to getQPTypes: " + response.getStatus() + " - " + content)
    ...
    if (response.getStatus() == Response.Status.OK.getStatusCode()) {
          ...
          JSONConversions.optionalList(obj, x => (x) match { case null => null; case o: org.codehaus.jettison.json.JSONObject => cache.objectFor(o).getOrElse(cache.seen(o, com.trafigura.tradecapture.internal.refinedmetal.QPType.fromJson(o, cache))) })
	      } catch {
          case ex : Exception => {
...
// Wrap a QPTypeService as a QPTypeServiceResource (e.g. to be
// exposed by a JAX-RS implementation).

class QPTypeServiceResourceStub(val target: QPTypeService, filters: java.util.List[ServiceFilter])
    extends AbstractResourceStub(filters)
    with QPTypeServiceResource {

  def this(target: QPTypeService) = this(target, new java.util.ArrayList[ServiceFilter])
  ...
  def getQPTypes(_userHeader:String): Response = {

    try {
    val params = new JSONObject()
     val res = 
      QPTypeServiceJSON.dispatch(target, filters.toSeq, "GetQPTypes", params, buildSecurityContext(_userHeader))
