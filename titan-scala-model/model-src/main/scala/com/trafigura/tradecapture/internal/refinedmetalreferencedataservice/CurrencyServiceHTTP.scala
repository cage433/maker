...
// JAX-RS support for CurrencyService


// A resource trait corresponding to CurrencyService

@Path("/CurrencyService")
trait CurrencyServiceResource {
  
  @Path("GetCurrencies")
  @POST  @Produces(Array("application/json"))
  def getCurrencies(@HeaderParam("User") _userHeader:String): Response
  
}


// Wrap a CurrencyServiceResource (provided by RESTEasy's client
// support) as a CurrencyService
//
// Note: will throw an exception on parse/status code errors, exception will contain details of the error
//
class CurrencyServiceResourceProxy(val resource: CurrencyServiceResource) extends CurrencyService {

  val log = org.slf4j.LoggerFactory.getLogger(this.getClass.getName)

  
  def getCurrencies() = getCurrencies(null)
  def getCurrencies( securityContext:SecurityContext = null): List[com.trafigura.tradecapture.internal.refinedmetal.Currency] = {
    val _userHeader = if (securityContext != null) { securityContext.user } else { null }
    val response = resource.getCurrencies(_userHeader)
    val content = response.asInstanceOf[ClientResponse[String]].getEntity(classOf[String])

    log.debug("Completed call to getCurrencies: " + response.getStatus() + " - " + content)
    ...
    if (response.getStatus() == Response.Status.OK.getStatusCode()) {
          ...
          JSONConversions.optionalList(obj, x => (x) match { case null => null; case o: org.codehaus.jettison.json.JSONObject => cache.objectFor(o).getOrElse(cache.seen(o, com.trafigura.tradecapture.internal.refinedmetal.Currency.fromJson(o, cache))) })
	      } catch {
          case ex : Exception => {
...
// Wrap a CurrencyService as a CurrencyServiceResource (e.g. to be
// exposed by a JAX-RS implementation).

class CurrencyServiceResourceStub(val target: CurrencyService, filters: java.util.List[ServiceFilter])
    extends AbstractResourceStub(filters)
    with CurrencyServiceResource {

  def this(target: CurrencyService) = this(target, new java.util.ArrayList[ServiceFilter])
  ...
  def getCurrencies(_userHeader:String): Response = {

    try {
    val params = new JSONObject()
     val res = 
      CurrencyServiceJSON.dispatch(target, filters.toSeq, "GetCurrencies", params, buildSecurityContext(_userHeader))
