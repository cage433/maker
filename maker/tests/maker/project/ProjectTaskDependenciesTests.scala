  ...
  test("TestCompile by default doesn't depend on upstream modules TestCompile"){
    withTempDir{
      dir => 
        ...
        assert(
          C.testCompileTaskBuild.graph.upstreams(TestCompileTask(C, C)).contains(TestCompileTask(C, A)),
  ...
  test("Upstream module tests are associated tasks"){
    withTempDir{
      dir => 
        ...
        List(A, B, C).foreach{
          proj => 
            assert(proj.testTaskBuild.graph.nodes.exists{
              case RunUnitTestsTask(_, _, `proj`, _) => true
              case _ => false
            })
        }
        ...
        assert(C.testTaskBuild.graph.edges.contains(Edge(TestCompileTask(C, A), TestCompileTask(C, C))))
