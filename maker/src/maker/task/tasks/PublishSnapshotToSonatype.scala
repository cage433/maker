...
case class PublishSnapshotToSonatype(baseProject : BaseProject, version : String) extends Task
with SonatypeTask with EitherPimps{
  ...
  def name = s"Publish $baseProject snapshot to Sonatype"

  def upstreamTasks = 
    PublishLocalTask(baseProject, baseProject.allUpstreamModules, version, signArtifacts = true) :: Nil

  def exec(results : Iterable[TaskResult], sw : Stopwatch) = {
    val toUpload : Seq[(File, ContentType)] = (baseProject.publishLocalPomFile(version), ContentType.APPLICATION_XML) +: {
      findJars(baseProject.publishLocalJarDir(version)).map((_, ContentType.DEFAULT_BINARY))
    }
  ...
  private def uploadToSonatype(file : File, contentType : ContentType) : Either[ErrorMessage, String] = {
    val url = 
      "https://oss.sonatype.org/content/repositories/snapshots/" + 
        baseProject.organization.getOrElse(???).replace('.', '/') + "/" + 
        baseProject.artifactId + "/" + version + "/" + file.basename
