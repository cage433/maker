...
class FuturesTests extends TestMarketTest with TestNGSuite {
  ...
  val inst = Future(mkt, p, K, volume)
  ...
  def testRBOBFutureChangeExplanation() {
	...
	def testFuturesMtmUnderPerturbedEnvironment{
		...
		val shifts : Map[AtomicDatumKey, Quantity] = Map.empty + (ForwardPriceKey(mkt, p) -> dP)
  ...
  def testExplanation() {
    ...
    assertEquals(explain.format(1), "((Shanghai Zinc.DEC 2009 - 105.00 CNY/MT) × 1,000.00 MT)")
    val ex = "((99.00 CNY/MT - 105.00 CNY/MT) × 1,000.00 MT)"
